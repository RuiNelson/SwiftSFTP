...
- **Resumable Transfers**: `multiUpload` / `multiDownload` take a `resumable:` argument that lets an interrupted multi-worker transfer continue instead of restarting, keeping its state inside the partial file rather than in a database; see [Resumable parallel transfers](Documentation/UserGuide.md#resumable-parallel-transfers).
...
- **No resume, unless it is asked for.** By default an interrupted `multiUpload`/`multiDownload` cannot continue from where it left off and must restart from the beginning. Passing `resumable: .resumable` changes that, at the cost of a partial file that outlives a failed attempt; see below.
