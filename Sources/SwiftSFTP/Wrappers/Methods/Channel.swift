...
///
/// The `channelType` argument selects the channel flavour to open. The SSH2
/// protocol defines `session`, `direct-tcpip`, and `tcpip-forward`; custom
/// types are permitted by the protocol but depend on server support. `message`
/// carries any type-specific payload (for example, the originator address
/// required by forwarded channels).
///
/// - Parameters:
///   - session: The session that will own the channel.
///   - channelType: Channel type to open (e.g. `session`, `direct-tcpip`).
///   - windowSize: Maximum amount of unacknowledged data the remote host is
///     allowed to send before receiving an `SSH_MSG_CHANNEL_WINDOW_ADJUST`.
///   - packetSize: Maximum number of bytes the remote host is allowed to send
///     in a single data or extended-data packet.
...
///   socket send, channel failure, or `EAGAIN` when the session is
...
///
/// Traffic from the local client to the SSH server remains encrypted; the
/// segment from the SSH server to `host:port` travels in cleartext. Use
...
///
/// This is the stream-local (OpenSSH `direct-streamlocal@openssh.com`)
/// counterpart to ``ChannelDirectTCPIP(session:host:port:sourceHost:sourcePort:)``:
/// traffic between the local client and the SSH server is encrypted, while
/// the segment between the SSH server and the remote socket travels in
/// cleartext. It is typically used to reach agent or other local services
/// exposed on the remote host.
///
/// - Parameters:
///   - session: The session that will own the channel.
...
///   - host: Specific address on the remote host to bind to. Pass `nil` to
///     bind to all available addresses (`0.0.0.0`).
///   - port: Port to bind to on the remote host. Pass `0` to let the remote
///     host pick the first available dynamic port; the chosen port is
///     returned in `boundPort`.
///   - queueMaxSize: Maximum number of pending connections to queue before
///     the server starts rejecting further attempts.
/// - Returns: A tuple containing the new ``LibSSH2Listener`` and the actual
///   port bound on the remote host.
/// - Throws: ``LibSSH2Error`` if the request fails (allocation, socket send,
///   protocol, request denied, or `EAGAIN` for non-blocking sessions).
...
///
/// Call this repeatedly after ``ChannelForwardListen(session:host:port:queueMaxSize:)``
/// to drain queued connections, typically from a dedicated thread or a
/// dispatch source in non-blocking mode.
///
...
/// - Throws: ``LibSSH2Error`` with `.nullPointer` if no connection is ready or
///   the call fails.
...
///
/// This only applies to channel types that support environment propagation
/// (for example, session channels about to start a shell or command). The
/// server may ignore the request despite reporting success.
///
/// - Parameters:
///   - channel: The channel that will carry the environment request.
///   - variableName: Name of the environment variable to set.
///   - value: Value to assign to the variable.
/// - Throws: ``LibSSH2Error`` on failure (allocation, socket send, request
...
///
/// On success, the remote side starts an agent listener for the duration of
/// the SSH session. The session must first register a `LIBSSH2_CALLBACK_AUTHAGENT`
/// callback through `libssh2_session_callback_set2`; that callback fires
...
///
/// Only some channel types accept PTY requests; the server may ignore the
/// request on incompatible channels despite returning success.
///
/// - Parameters:
...
/// host accepts the X11 forwarding and opens a connection back to the local
/// X11 proxy.
///
/// - Parameters:
///   - channel: The channel on which to request X11 forwarding.
///   - singleConnection: If `true`, only a single X11 connection will be
///     forwarded for the life of the session.
...
///
/// The signal name must be the same as the POSIX constant without the
/// leading `SIG` (for example `TERM`, `HUP`, `KILL`). Some servers or
/// platforms may not implement signal delivery and will ignore the request.
///
/// - Parameters:
...
/// the standard process services. `message` carries any request-specific
/// payload: the command line for `exec`, the subsystem name for `subsystem`,
/// or nothing for `shell`.
///
/// - Parameters:
...
///
/// Every channel has a standard I/O substream at `streamID == 0` and may
/// have up to 2^32 extended-data substreams. The SSH2 protocol defines
...
/// - Returns: The bytes read. An empty ``Data`` value means no payload was
///   available at this time, not an error.
/// - Throws: ``LibSSH2Error`` on failure, including `EAGAIN` for non-blocking
...
/// inspects buffered channel data and does not see if packets are waiting
/// to be processed on the transport.
///
/// - Parameters:
///   - channel: The channel to poll.
///   - extended: If `true`, check the extended-data substream instead of
///     the standard I/O substream.
...
///   - `windowSize`: the number of bytes the remote end may still send
///     before exceeding the window limit,
///   - `readAvailable`: the number of bytes already buffered locally and
///     available to read,
///   - `initialWindowSize`: the window size that was negotiated when the
///     channel was opened.
...
///
/// If `adjustment` is below `LIBSSH2_CHANNEL_MINADJUST` and `force` is `false`
/// the value is queued for a later packet instead of being sent immediately.
/// The remote-side window size after the adjustment is returned.
///
/// - Parameters:
///   - channel: The channel to adjust.
///   - adjustment: Number of bytes to add to the receive window.
///   - force: If `true`, send the adjustment immediately even when it is
///     below the minimum threshold.
/// - Returns: The new size of the receive window as understood by the
///   remote end.
/// - Throws: ``LibSSH2Error`` on failure, including `EAGAIN` for non-blocking
///   sessions.
...
/// throughput on large payloads, pass buffers of at least 32 KiB so the
/// call can pack them into single SSH protocol packets.
///
/// - Parameters:
...
/// - Throws: ``LibSSH2Error`` on failure (allocation, socket send, channel
///   closed, EOF sent, or `EAGAIN` for non-blocking sessions).
...
/// - Parameter channel: The channel to inspect.
/// - Returns: A tuple containing:
///   - `windowSize`: the number of bytes that may be safely written
...
///
/// This currently forwards to `libssh2_session_set_blocking`, so the change
/// applies to the whole session and all of its channels, not just the one
/// passed in.
///
/// - Parameters:
///   - channel: The channel whose blocking mode should be changed.
///   - blocking: If `true`, make the session block on I/O; if `false`,
///     make it return `EAGAIN` when I/O would block.
...
/// `ignoreMode`:
/// - `LIBSSH2_CHANNEL_EXTENDED_DATA_NORMAL` (`0`): queue extended data for
///   reading,
/// - `LIBSSH2_CHANNEL_EXTENDED_DATA_MERGE` (`2`): merge extended data with
///   ordinary data so reads pull from all substreams in FIFO order,
/// - `LIBSSH2_CHANNEL_EXTENDED_DATA_IGNORE` (`1`): discard extended data as
///   it arrives.
///
/// - Parameters:
...
///
/// Pass `streamID == 0` for the standard I/O substream or `1` for stderr.
/// The libssh2 constants `LIBSSH2_CHANNEL_FLUSH_EXTENDED_DATA` (`-1`) and
/// `LIBSSH2_CHANNEL_FLUSH_ALL` (`-2`) flush all extended-data substreams
/// or every substream at once.
///
/// - Parameters:
///   - channel: The channel to flush.
///   - streamID: Substream to flush, or one of the `LIBSSH2_CHANNEL_FLUSH_*`
///     constants to flush multiple substreams.
/// - Throws: ``LibSSH2Error`` on failure, including `EAGAIN` for non-blocking
///   sessions.
...
///
/// The exit status is only available after the remote side has closed the
/// channel. This wrapper uses the `libssh2_channel_get_exit_status` API,
/// which is a macro and may be updated in newer libssh2 releases to query
/// additional status structures.
///
/// - Parameter channel: The channel whose remote exit status is requested.
/// - Returns: The exit status reported by the remote host, or `0` if it is
///   not yet available.
...
///
/// The signal name is provided without the leading `SIG` prefix. The
/// accompanying message and language tag are optional and may be absent if
/// the server did not include them.
///
/// - Parameter channel: The channel whose remote exit signal is requested.
/// - Returns: A tuple containing:
///   - `exitSignal`: the signal name (without `SIG` prefix), or `nil` if
///     the remote process exited cleanly,
...
///
/// Remote processes typically interpret this as a closed stdin descriptor.
/// The remote side may still send data back until it closes the channel
/// itself; use ``ChannelWaitEOF(channel:)`` to block until that happens.
///
...
///
/// Sends an `SSH_MSG_CLOSE` packet to the remote host. The remote side may
/// still send data until it sends its own close message; follow up with
...
///
/// Typically called after ``ChannelClose(channel:)`` so the exit status
/// returned by ``ChannelGetExitStatus(channel:)`` and
...
///
/// If the channel has not yet been closed with ``ChannelClose(channel:)``
/// the close is sent automatically so that the remote end can free its
/// own resources. The ``LibSSH2Channel`` handle is no longer usable after a
/// successful call.
///
