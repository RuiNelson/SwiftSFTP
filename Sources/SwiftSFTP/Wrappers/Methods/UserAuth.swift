...
private let _userAuthPasswordChangeCallback: @convention(c) (
    ...
    guard
        let sessionPointer,
        let box: LibSSH2PasswordChangeBox = _libssh2CallbackBox(abstract) else {
        return
    }
...
private let _userAuthPublicKeySignCallback: @convention(c) (
    ...
    guard
        let sessionPointer,
        let box: LibSSH2PublicKeySignBox = _libssh2CallbackBox(abstract) else {
        return -39
    }
...
private let _userAuthSecurityKeySignCallback: @convention(c) (
    ...
    -> Int32 =
    { sessionPointer, signatureInfo, data, dataLength, algorithm, flags, application, keyHandle, keyHandleLength, abstract in
        guard
            let sessionPointer,
            let signatureInfo,
            let box: LibSSH2SecurityKeySignBox = _libssh2CallbackBox(abstract) else {
            return -39
        }
...
///
/// Sends an `SSH_USERAUTH_NONE` request to the server. The server rejects the request and returns the set of
/// authentication methods it is willing to accept for `username`. Use this to pick an appropriate authenticator before
/// calling one of the `UserAuth*` functions. Most servers do not allow the username to change between this call and the
/// actual authentication attempt, so use the same value for both.
///
/// - Parameters:
///   - session: The session to query.
///   - username: The username the authentication will eventually use.
/// - Returns: The comma-separated list of method names parsed into individual strings, or `nil` if the request fails or
/// the server unexpectedly accepts the none request.
...
///
/// Retrieves the banner message sent by the server along with the most recent `SSH_USERAUTH` response. Call this after
/// a
/// ``UserAuthList(session:username:)`` or any `UserAuth*` attempt. The
/// returned string is empty when the server has not advertised a banner.
///
/// - Parameter session: The session whose banner should be read.
/// - Returns: The UTF-8 banner text, or an empty string when no banner has been received.
/// - Throws: ``LibSSH2Error`` with `.missingUserAuthBanner` when no authentication attempt has yet been made or the
/// server did not send a banner.
...
/// - Returns: `true` once a user-authentication attempt has succeeded, otherwise `false`.
...
///
/// Attempts basic password authentication. Many servers that appear to accept password authentication actually disable
/// it and route the challenge through keyboard-interactive (PAM or another backend) — prefer
/// ``UserAuthKeyboardInteractive(session:username:handler:)`` when the server advertises that method.
///
/// - Parameters:
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - password: Password for `username`.
///   - changeHandler: Optional callback invoked when the server reports the password has expired. The handler must
/// return the new password; when `nil` is passed and the server requires a change, authentication fails with
/// ``LibSSH2Error/passwordExpired``.
/// - Throws: ``LibSSH2Error`` on failure (allocation, socket send, password expired, authentication failed, or `EAGAIN`
/// for non-blocking sessions).
...
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - publicKeyPath: Path to the public key file, or `nil` to extract the public key from `privateKeyPath`.
///   - privateKeyPath: Path to the PEM-encoded private key file.
///   - passphrase: Passphrase to decrypt the private key, or `nil` when the key is not encrypted.
/// - Throws: ``LibSSH2Error`` on failure (file, allocation, socket send, socket timeout, public-key unverified,
...
///
/// Performs `ssh-hostbased` authentication. The client proves the request originates from a trusted client machine by
/// signing the session identifier with `privateKeyPath` and presenting the local user name and remote host name to the
/// server. The server then checks `~/.shosts`, `~/.rhosts`, or `/etc/hosts.equiv` on its end.
///
/// - Parameters:
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - publicKeyPath: Path to the host's public key file, or `nil` to extract it from the private key file.
///   - privateKeyPath: Path to the host's private key file.
///   - passphrase: Passphrase to decrypt the private key, or `nil` when the key is not encrypted.
///   - hostname: The host name the client is connecting from. Must resolve to an address listed in the server's
/// `hosts.equiv` or per-user `shosts`/`rhosts` file.
///   - localUsername: The user name on the local client machine.
/// - Throws: ``LibSSH2Error`` on failure (file, allocation, socket send, authentication failed, or `EAGAIN` for
...
///   - passphrase: Passphrase to decrypt the private key, or `nil` when the key is not encrypted.
/// - Throws: ``LibSSH2Error`` on failure (allocation, socket send, socket timeout, public-key unverified,
/// authentication failed, or `EAGAIN` for non-blocking sessions).
...
///
/// Use this overload when the private key is not available as a file or in-memory PEM block — for example, when it
/// lives in a Secure Enclave, the Keychain, or a custom hardware module. libssh2 supplies the data to sign and the
/// callback returns the signature in SSH wire format.
///
/// - Parameters:
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - publicKeyData: Raw public key blob (the `ssh-...` key as it appears in `authorized_keys`).
///   - signHandler: Closure invoked by libssh2 to produce the signature. The closure receives the session and the data
/// to sign; throw to abort authentication with the supplied error. Return `Data()` to indicate the key cannot sign the
/// request (libssh2 may then try another key).
/// - Throws: An error thrown by `signHandler`, or ``LibSSH2Error`` on failure (allocation, socket send, authentication
/// failed, or
...
///
/// Performs `keyboard-interactive` (challenge/response) authentication. The server sends one or more prompts and the
/// closure returns the matching responses. Many servers emit a single "password:" prompt even when the underlying
/// scheme is something more involved (PAM, one-time passwords, smart cards).
///
/// - Parameters:
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - handler: Closure invoked for each challenge the server presents. The challenge is delivered as a
///     ``LibSSH2KeyboardInteractiveChallenge``; return the
///     corresponding response strings, one per prompt. Empty strings are substituted when fewer responses are returned
/// than there are prompts.
/// - Throws: ``LibSSH2Error`` on failure (allocation, socket send, authentication failed, or `EAGAIN` for non-blocking
/// sessions).
...
///   - session: The session to authenticate.
///   - username: Remote user name to authenticate as.
///   - publicKeyData: Raw public key blob associated with the authenticator's key handle.
///   - privateKeyData: Contents of the OpenSSH security-key private key file (typically generated with `ssh-keygen -t
/// ed25519-sk`).
///   - passphrase: Passphrase to decrypt the private key file, or
///     `nil` when the key is not encrypted.
///   - signHandler: Closure that drives the FIDO2 authenticator to produce a signature. The request is delivered as a
///     ``LibSSH2SecurityKeySigningRequest``; return the
///     ``LibSSH2SecurityKeySignatureInfo`` populated with the
///     authenticator's response. Throw to abort authentication with the supplied error.
/// - Throws: An error thrown by `signHandler`, or ``LibSSH2Error`` on failure (allocation, socket send, authentication
/// failed, or
///   `EAGAIN` for non-blocking sessions).
...
///
/// Low-level entry point that mirrors `libssh2_sign_sk`. Callbacks registered through `libssh2_session_callback_set2`
/// with
/// `LIBSSH2_CALLBACK_AUTHAGENT_SIGN` or security-key agent paths
/// receive a `LIBSSH2_PRIVKEY_SK` structure through the `abstract` pointer; this function lets Swift code create a
/// signature from that data without exposing the underlying C layout.
///
/// - Parameters:
///   - session: The session performing the signing operation.
///   - data: The data to sign.
///   - abstract: Pointer to the `LIBSSH2_PRIVKEY_SK` structure supplied by libssh2. The structure is fully populated by
/// libssh2 and includes the algorithm, flags, application string, key handle, and signing callback to use.
