...
///
/// This typically initializes the crypto library. It uses a global state
...
///
/// Uses the memory allocation callbacks registered with the session, if
/// any, otherwise calls the standard `free()`. Mostly useful on Windows
/// when libssh2 and the application are linked against different C
/// runtimes.
///
/// - Parameters:
...
///
/// - Parameter requiredVersionNumber: A `LIBSSH2_VERSION_NUM`-style
///   version number, or `0` for the unconditional version string.
...
///
/// - Returns: A ``LibSSH2CryptoEngine`` value identifying the active
///   crypto backend.
