...
/// response from the server. `intervalSeconds` is the number of seconds that can pass without any I/O; use `0` to
/// disable keepalives. To avoid busy-loop corner-cases, an interval of `1` is treated as `2`.
///
/// Non-blocking applications are responsible for sending the keepalive messages themselves using
/// ``KeepAliveSend(session:)``.
///
/// - Parameters:
///   - session: The session to configure keepalives on.
///   - wantsReply: Whether the keepalive messages should request a reply from the server.
///   - intervalSeconds: Number of seconds that can pass without I/O before a keepalive is sent. `0` disables
/// keepalives; `1` is treated as `2`.
...
///
/// The return value indicates how many seconds may elapse before the next call to this function is required.
///
/// - Parameter session: The session to send a keepalive on.
/// - Returns: Seconds to wait before the next keepalive send is required.
/// - Throws: ``LibSSH2Error`` with ``LibSSH2Error/socketSend(_:)`` on I/O errors.
